...
export const UserIcon = styled(UserIconSvg).attrs(({ theme }) => ({
	width: theme.rem(48),
	height: theme.rem(48),
...
export const Background = styled.View<{ $logged: boolean }>`
	...
	justify-content: center;
	background-color: ${({ theme }) => theme.grayscale.gray900};
	border-width: ${({ theme, $logged }) => ($logged ? theme.rem(2) : theme.rem(1))};
	border-color: ${({ theme, $logged }) => ($logged ? theme.grayscale.gray900 : theme.grayscale.gray600)};
`;
